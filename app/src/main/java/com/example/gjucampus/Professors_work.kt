...
class Professors_work : AppCompatActivity(), NavigationView.OnNavigationItemSelectedListener {
    private lateinit var drawerLayout: DrawerLayout
    override fun onCreate(savedInstanceState: Bundle?) {
        super.onCreate(savedInstanceState)
        setContentView(R.layout.activity_professors_work)
        ...
        if (savedInstanceState == null) {
            supportFragmentManager.beginTransaction()
                .replace(R.id.Student_work, Teacher_Home_fragment()).commit()
    ...
    override fun onNavigationItemSelected(item: MenuItem): Boolean {
        when (item.itemId) {
            ...
            R.id.Profile -> supportFragmentManager.beginTransaction()
                .replace(R.id.Student_work, Teacher_Profile()).commit()
